...
YoloV8::YoloV8(const std::string &onnxModelPath, const std::string &trtModelPath, const YoloV8Config &config)
  ...
  // Build the onnx model into a TensorRT engine file, cache the file to disk, and then load the TensorRT
  // engine file into memory. If the engine file already exists on disk, this function will not rebuild but
  // only load into memory. The engine file is rebuilt any time the above Options are changed.
  if (!onnxModelPath.empty()) {
    // Build the ONNX model into a TensorRT engine file
    auto succ = m_trtEngine->buildLoadNetwork(onnxModelPath, SUB_VALS, DIV_VALS, NORMALIZE);
    if (!succ) {
      const std::string errMsg =
          "Error: Unable to build or load the TensorRT engine from ONNX model. "
...
std::vector<std::vector<cv::cuda::GpuMat>> YoloV8::preprocess(const cv::cuda::GpuMat &gpuImg) {
  ...
  if (resized.rows != inputDims[0].d[1] || resized.cols != inputDims[0].d[2]) {
    ...
    resized =
        Engine<float>::resizeKeepAspectRatioPadRightBottom(rgbMat, inputDims[0].d[1], inputDims[0].d[2]);
  }
...
std::vector<Object> YoloV8::detectObjects(const cv::cuda::GpuMat &inputImageBGR) {
  ...
  if (numOutputs == 1) {
    ...
    // Since we have a batch size of 1 and only 1 output, we must convert the output from a 3D array to a 1D
    // array.
    ...
    // Segmentation
    // Since we have a batch size of 1 and 2 outputs, we must convert the output from a 3D array to a 2D
    // array.
...
std::vector<Object> YoloV8::postProcessSegmentation(std::vector<std::vector<float>> &featureVectors) {
  ...
  if (!masks.empty()) {
    ...
    for (size_t i = 0; i < indices.size(); i++) {
      ...
      dest = 1.0 / (1.0 + dest);
      dest = dest(roi);
      cv::resize(dest, mask, cv::Size(static_cast<int>(m_imgWidth), static_cast<int>(m_imgHeight)),
                 cv::INTER_LINEAR);
...
void YoloV8::drawObjectLabels(cv::Mat &image, const std::vector<Object> &objects, unsigned int scale) {
  ...
  if (!objects.empty() && !objects[0].boxMask.empty()) {
    ...
    for (const auto &object : objects) {
      ...
      cv::Scalar color =
          cv::Scalar(COLOR_LIST[colorIndex][0], COLOR_LIST[colorIndex][1], COLOR_LIST[colorIndex][2]);
